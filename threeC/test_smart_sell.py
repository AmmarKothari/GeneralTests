...
for pair in PAIRS_OF_INTEREST:
    ...
    for deal_id in EXISTING_DEALS:
        success, response = py3cw.request(
            entity="smart_trades_v2", action="get_by_id", action_id=str(deal_id)
        )
        if success:
            ...
            success, response = py3cw.request(
                entity="smart_trades_v2", action="cancel", action_id=str(deal_id)
            )

    deal = new_deals[0]
    success, pair_market_data = py3cw.request(
        entity="accounts",
    ...
    amount_in_account = account.get_coin_in_account(
        pair.split("_")[1], exchange_account_id
    )
    ...
    if deal_amount < float(deal["amount"]):
        print(
            f'There was less coin than the original deal requested. {deal_amount} < {deal["amount"]}'
        )

    units_to_buy = math.floor(deal_amount / float(pair_market_data["lotStep"])) * float(
        pair_market_data["lotStep"]
    )
    sale_price = math.floor(
        float(deal["sale_price"]) / float(pair_market_data["priceStep"])
    ) * float(pair_market_data["priceStep"])
    ...
    success, response = py3cw.request(
        entity="smart_trades_v2", action="new", payload=payload
    )
    if success:
        print(payload)
