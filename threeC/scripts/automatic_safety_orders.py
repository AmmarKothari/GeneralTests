    ...
    for base_currency in CURRENCY_SETTINGS:
        ...
        for smart_deal in all_smart_deals:
            if smart_deal.get_base_currency() != base_currency:
                print(
                    f"Base currency of deal does not match {smart_deal.get_base_currency()} != {base_currency}"
                )
                continue
            if not smart_deal.is_open():
                print(
                    f"Deal {smart_deal.get_id()} ({smart_deal.get_pair()}) has not been opened so can't add funds."
                )
                continue
            ...
            if time_delta.total_seconds() < UPDATE_TIME_THRESHOLD:
                print(
                    f"{smart_deal.get_pair():<10} deal has been updated recently. ({time_delta.seconds}s < {UPDATE_TIME_THRESHOLD}s) Not adding safety order."
                )
                continue
            ...
            if success:
                ...
                continue
            amount = CURRENCY_SETTINGS[base_currency]["base_order_size"] / float(
                response["last"]
            )
            ...
            try:
                ...
                continue
            print(
                f"{smart_deal.get_id()}: Created a new safety order for {smart_deal.get_pair()}"
            )
