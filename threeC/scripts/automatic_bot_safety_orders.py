...
for account in [
    accounts.get_account_from_name(account_name)
    ...
    for base_currency in CURRENCY_SETTINGS:
        ...
        if coin_in_account < min_in_account:
            print(
                f"{coin_in_account:.6f} < {min_in_account:.6f} so not going to open more safety orders"
            )
            continue
        ...
        for bot_deal in all_bot_deals:
            if bot_deal.get_base_currency() != base_currency:
                print(
                    f"{bot_deal.get_id()}: Base currency of deal does not match {bot_deal.get_base_currency()} != {base_currency}"
                )
                continue

            if bot_deal.get_active_safety_orders() > ACTIVE_SAFETY_ORDER_MIN:
                print(
                    f"{bot_deal.get_id()}: Active safety orders more than minimum {bot_deal.get_active_safety_orders()} <= {ACTIVE_SAFETY_ORDER_MIN}"
                )
                continue
            ...
            if time_delta.total_seconds() < UPDATE_TIME_THRESHOLD:
                print(
                    f"{bot_deal.get_id()}: {bot_deal.get_pair()} deal has been updated recently. ({time_delta.seconds}s < {UPDATE_TIME_THRESHOLD}s) Not adding safety order."
                )
                continue
            ...
            if deal_count > MAX_NUMBER_OF_DEALS:
                print(
                    f"Exceeded the max number of deals for currency ({MAX_NUMBER_OF_DEALS})"
                )
                break
            ...
            try:
                ...
                continue
            amount = float(bot_deal.deal["base_order_volume"]) / float(
                response["orderbook_price"]
            )
            ...
            buy_price_raw = (
                float(response["orderbook_price"])
                * CURRENCY_SETTINGS[base_currency]["ratio_of_order_price"]
            )
            # Taking the slightly lower price
            buy_price = math.floor(
                buy_price_raw / float(response["limits"]["priceStep"])
            ) * float(response["limits"]["priceStep"])
            ...
            try:
                ...
                deal_count -= 1
                continue
            new_safety_order_msgs.append(
                f"{bot_deal.get_id()}: Created a new safety order for {bot_deal.get_pair()}"
            )
