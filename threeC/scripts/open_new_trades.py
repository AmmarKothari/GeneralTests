...
for deal in all_deals:
    ...
    units_to_buy = math.floor(
        float(deal["amount"]) / float(response["lotStep"])
    ) * float(response["lotStep"])
    sale_price = math.floor(
        float(deal["sale_price"]) / float(response["priceStep"])
    ) * float(response["priceStep"])
    ...
    payload = {
        ...
        "position": {
            ...
            "conditional": {"price": {"value": sale_price}, "order_type": "market"},
        },
