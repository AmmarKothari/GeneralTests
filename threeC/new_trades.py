...
ACCUMULATE_KEYS = ['id', 'base_order_volume', 'bought_amount', 'bought_volume', 'bot_id',
                   'bought_average_price', 'safety_order_volume', 'actual_profit',
                   'actual_usd_profit', 'bot_name', 'reserved_base_coin', 'reserved_second_coin',
                   'completed_manual_safety_orders_count', 'sold_amount', 'current_active_safety_orders_count',
                   'sold_volume', 'completed_safety_orders_count', 'sold_average_price', ]
...
for pair in all_trade_pairs:
    ...
    weighted_price = sum([float(avg) * float(amt) for avg, amt in zip(trade_pairs[pair]['bought_average_price'], trade_pairs[pair]['bought_amount'])]) / amount_bought
    ...
    success, all_market_pairs = py3cw.request(entity='accounts', action='market_pairs', payload={'market_code': 'binance'})

    try:
        success, response = py3cw.request(entity='accounts', action='currency_rates', payload={'pair': pair, 'market_code': 'binance'})
    ...
    id += 1
    print(f'ID: {id:<3} | Pair: {pair:<10} | Sell {amount_bought:<15.3f} {alt:<5} at {sale_price:<10.8f} {base} | {percent_increase*100:.2f} % increase')

    new_deals.append({'pair': pair, 'id': id, 'amount': amount_bought, 'sale_price': sale_price, 'percent_increase': percent_increase*100})
